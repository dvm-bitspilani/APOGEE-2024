...
export default function ControlsRobot({
  robot,
  robotHead,
  robotBall,
  drone,
  ...
  useFrame((state) => {
    ...
    //elevator Update
    elevator.current.position.y =
      robot.current.position.y - initialPositionOfElevator;
    ...
    if (pos.y < -16.5) positionY = -16.5;
    else if (pos.y < 1.5 && robotIsOnFirstPlat) positionY = 1.5;
    else {
      positionY = pos.y - mouse.y * 5;
    }

    drone.current.lookAt(
      new Vector3(

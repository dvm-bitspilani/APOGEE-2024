...
export default function Decals({
    ...
    const [react,three,blender,greensock,rapier] = useTexture(['./textures/react.png','./textures/three.png','./textures/blender.png','./textures/gsap.png','./textures/rapier.png'])

    const initNewDecal = (decalArgs,color,map,toneMapped)=>{
        toneMapped === undefined ? toneMapped = true : 0

        return (
            <Decal position={[decalArgs.x,decalArgs.y,decalArgs.z]} rotation={[0,decalArgs.rot,0]} scale={decalArgs.scale}>
                <meshStandardMaterial color={color} map={map} toneMapped={toneMapped}
                    transparent polygonOffset polygonOffsetFactor={-10} 
                 />
            </Decal>
        )
    }
